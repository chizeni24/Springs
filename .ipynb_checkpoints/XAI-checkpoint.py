...
def Load_Springs(df_AI,Approved_date,basein,skip_rows,base_dir,filename):
    ...
    Spring_Flow
    df_numeric_values_SanM = pd.to_numeric(Spring_Flow['SF$[m^3/s]$'], errors='coerce')
    sf_data = df_numeric_values_SanM.to_frame()
    ...
    #San antonio date approved! 2020-10-11
...
def GAA_tuned(Model, objective_parameters,X_train,y_train,save_extra ):
...
def Data_training(Model, path_to_Save):
    ...
    print("############ Model Accuracy-Train set############")
...
def interaction_collage(dataframe,F,S,T,FR, save_at):
    ...
    shap.dependence_plot(F,shap_values, dataframe[Features].to_numpy(),Features, ax=ax1, show=False,
                         interaction_index=F)

    ax2 = fig.add_subplot(gs[1, 0])

    shap.dependence_plot(S,shap_values, dataframe[Features].to_numpy(),Features, ax=ax2, show=False,
                         interaction_index=S)

    ax3 = fig.add_subplot(gs[0, 1])

    shap.dependence_plot(T, shap_values, dataframe[Features].to_numpy(),Features, ax=ax3, show=False,
                         interaction_index=T)
    ...
    shap.dependence_plot(FR, shap_values, dataframe[Features].to_numpy(),Features, ax=ax4, show=False,
                         interaction_index=FR)


    plt.savefig(save_at +str('/Interaction.jpg'), format='jpeg', dpi=300, bbox_inches ='tight')
...
def RCP(rcp_dataframe,df_AI, model, name, save_to): 
    colour = ''
    def run_scenerio(rcp_dataframe,df_AI, model, name, save_to, colour):
        
        date_rng = pd.date_range(start='1/08/2006', end='01/03/2100', freq='W', closed='left')
        ...
        for row in df_future.itertuples():
            ...
            if i == (len(date_rng)-1):
                break
        Y_future =df_AI['SF$[m^3/s]$'][df_future['SF$[m^3/s]$'][:str(Approved_date)].index].sort_values(ascending=True).to_numpy()

        Y_future_Pred = df_future['SF$[m^3/s]$']['2006-01-01':str(Approved_date)].sort_values(ascending=True).to_numpy()
        ...
        print("########################################")
        print(" ")
        # df_future['SF$^{lag1}$ $[m^3/s]$'] = df_future['SF$^{lag1}$ $[m^3/s]$'].astype(float)
        ...
        ax1.scatter(Y_future, 
                    df_future['SF$[m^3/s]$']['2006-01-01':str(Approved_date)].sort_values(ascending=True).to_numpy(),
                    marker='o', color = 'black')

        RSQ = np.round((r2_score(Y_future, 
                    df_future['SF$[m^3/s]$']['2006-01-01':str(Approved_date)].sort_values(ascending=True).to_numpy())),3)
        ...
        ax1.plot(df_AI[str(Approved_date):'2100-01-03'].index,
                 df_AI['SF$[m^3/s]$'][str(Approved_date):'2100-01-03'], '-', 
                 color = colour, lw = 3, label = 'Future_'+str(name)+ 'Projection')

        temp = pd.concat([df_AI['SF$[m^3/s]$'],df_future['SF$[m^3/s]$'][str(Approved_date):'2100-01-03']])
        ...
        #phase3[phase3 == 0] = 845*0.028316847
        #phase4 = np.zeros(len(temp))
        #phase4[phase4 == 0] = 842*0.028316847
        ...
        plt.fill_between(tw,   0, phase2[0], alpha=0.3, facecolor='khaki')
        ...
        #plt.fill_between(tw,      15. , phase5[0], alpha=0.15, facecolor='black')
        ...
        plt.text(tw[-1500],phase1[0]+9, r'RCP'+str(name), fontdict=font)

        #--------------Annotate the plot -------------####

        plt.axvline(x= datetime(2020, 11, 10), color = 'royalblue', lw = 4, linestyle='--')

        font = {'family': 'serif', 'color':  'black', 'weight': 'normal', 'size': 26,}      
        plt.text(tw[4500],phase1[0]+9, r'projected', fontdict=font)

        font = {'family': 'serif', 'color':  'black', 'weight': 'normal', 'size': 26,} 
        plt.text(tw[200],phase1[0]+9, r'historical', fontdict=font)

        #-------------------------------------------- FORMAT -------------------------------------------------------
        plt.tight_layout(pad=0.6)
        fig.savefig(save_to + str('/Decremental_SF.jpg'), format='jpeg', dpi=300, bbox_inches='tight')
    ...
    if name == 4.5:
        colour = 'darkgreen'
        df_future = run_scenerio(rcp_dataframe,df_AI, model, name, save_to, colour)
        df_future.to_excel(save_to+'/df_future_45.xlsx')
    else:
        colour = 'darkred'
        df_future =  run_scenerio(rcp_dataframe,df_AI, model, name, save_to, colour)
        df_future.to_excel(save_to+r'/df_future_85.xlsx') 
...
def MACA_data_SM(scenario,model):
    ...
    data_M_Hist.rename(columns = {'DATE' : 'Date',     'tasmin(K)' : 'Tmin[K]', 
                                  'tasmax(K)' : 'Tmax[K]' , 'pr(mm)' : 'Precip[mm]'}, \
                                  inplace = True)
