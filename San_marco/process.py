...
def NSE(predicted,actual):
	nsee_r = he.evaluator(he.nse, predicted,actual)
	return nsee_r
...
def print_results(model, X_train,y_train,save):
	...
	NSE = he.evaluator(he.nse, Y_train_pred, y_train) 
	...
	print("########################################")
	print(" ")
	bb = np.round([y_train.shape[0],Y_train_pred],2)
	np.savetxt(save+'/train.out', bb)
	...
	MAE = np.mean(np.abs(df_AI_Test['SF$[m^3/s]$'].sort_values(ascending=True).to_numpy() - 
						 df_AI_Test['SF$^{pred} [m^3/s]$'].sort_values(ascending=True).to_numpy()))
	NSE = he.evaluator(he.nse,df_AI_Test['SF$^{pred} [m^3/s]$'].sort_values(ascending=True).to_numpy(),
	...
	print("########################################")
	print(" ")
	df_AI_Test.to_excel(save+'/df_AI_Test.xlsx')
...
def Data_training(Model, path_to_Save):
	...
	#Fit the model 
	Model_SF =Model.fit(train_X, train_y)
	...
	MAE = np.mean(np.abs(df_AI_Test['SF$[m^3/s]$'].sort_values(ascending=True).to_numpy() - 
						 df_AI_Test['SF$^{pred} [m^3/s]$'].sort_values(ascending=True).to_numpy()))
	NSE = NSE(df_AI_Test['SF$^{pred} [m^3/s]$'].sort_values(ascending=True).to_numpy(),
			  df_AI_Test['SF$[m^3/s]$'].sort_values(ascending=True).to_numpy())
	print("############ Model Accuracy -Test set ############")
	print("MAE: %0.3f" % MAE,'m/s^3')
	print("NSE: %0.3f" % NSE)
...
def RCP(rcp_dataframe,df_AI, model, name, save_to): 
	...
	NSE = he.evaluator(he.nse, Y_future_Pred.tolist(), Y_future.tolist()) 
	...
	fig, ax1 = plt.subplots(1,1, figsize=(5,3.5))
	ax1.scatter(Y_future,df_future['SF$[m^3/s]$']['2009-01-01':str(Approved_date)].sort_values(ascending=True).to_numpy(),		
							marker='o', color = 'black')
...
def plot_projections(df_future, df_AI, name, save_to):
	...
	font = {'family': 'serif', 'color':  '#dfdfc1', 'weight': 'bold', 'size': 20} 
	plt.text(tw[-1], phase1[0]-0.2, r'CS1', fontdict=font)
	font = {'family': 'serif', 'color':  '#ffc49b', 'weight': 'bold', 'size': 20} 
	...
	plt.text(tw[-1000],phase1[0]+7.5, r'RCP'+str(name), fontdict=font)
	#--------------Annotate the plot -------------####
	...
	font = {'family': 'serif',
			'color':  colour, 
			'weight': 'bold', 'size': 26,} 
	plt.text(tw[4050],phase1[0]+7.5, 
			 r'Projected', fontdict=font)
	font = {'family': 'serif', 
			'color':  'black', 
			'weight': 'bold', 'size': 26,} 
	plt.text(tw[0],phase1[0]+7.5,
			 r'Historical', fontdict=font)
	#-------------------------------------------- FORMAT -------------------------------------------------------
	plt.tight_layout(pad=0.6)
	fig.savefig(save_to + str('/Decremental'+str(name)+'_SF.jpg'), format='jpeg', dpi=300, bbox_inches='tight')
	fig = fig 
...
def MACA_data_SM(scenario,model):
	...
	data_M_Hist['Tmin[C]'].interpolate(method = 'linear', 
									   limit_direction = 'both', inplace =True)
	data_M_Hist['Tmax[C]'].interpolate(method = 'linear', 
									   limit_direction = 'both', inplace =True)
	data_M_Hist['Precip[mm]'].interpolate(method = 'linear', 
										  limit_direction = 'both', inplace =True)
	return data_M_Hist
